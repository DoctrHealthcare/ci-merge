...
package_update(){
	...
	*_package_update)
		...
		(retry 2 npm install) || _exit $? "npm install failed (_package_update branch name)"
		(retry 4 npm audit fix --package-lock-only) || _exit $? "npm audit fix failed (_package_update branch name)"
